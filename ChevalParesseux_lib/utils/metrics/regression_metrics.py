...
def get_regression_significance(
    ...
    residuals = y_true - predictions
    residual_variance = np.sum(residuals**2) / (nb_observations - nb_features) if nb_observations > nb_features else 0
...
def get_durbin_watson(
    ...
    dw_stat = np.sum(residuals_diff**2) / np.sum(residuals**2) if np.sum(residuals**2) != 0 else 0
...
def breusch_pagan_test(
    ...
    lm_stat = (np.sum(residuals_squared_error ** 2) / 2) / np.var(residuals_squared_pred) if np.var(residuals_squared_pred) != 0 else 0
