...
class DecisionTreeClassifier(com.ML_Model):
    ...
    def get_impurity(self, y):
        ...
        """
        if self.params['criterion'] in self.available_entropies:
            impurity = self.available_entropies[self.params['criterion']](y)
        else:
            raise ValueError(f"Unknown criterion '{self.params['criterion']}'")
    ...
    def build_tree(self, X, y, depth=0):
        ...
        if (depth >= self.params['max_depth'] or num_labels == 1 or nb_samples < self.params['min_samples_split']):
            ...
            leaf_samples = count_occurrences(universe=self.labels_universe, series=pd.Series(y))
        ...
        samples = count_occurrences(universe=self.labels_universe, series=pd.Series(y))
