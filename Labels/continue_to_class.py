...
from Labels import common as com
...
class tripleBarrier_labeller(com.Labeller):
    ...
    def get_labels(
        ...
        series = self.processed_data.dropna().copy()
...
class lookForward_labeller(com.Labeller):
    ...
    #?____________________________________________________________________________________ #
    def process_data(self, window_smooth: int, lambda_smooth: float):
        ewma_series = aux.exponential_weightedMA(price_series=self.data, window=window_smooth, ind_lambda=lambda_smooth)
        self.processed_data = ewma_series
    ...
    def get_labels(
        ...
        Xdays_returns = (series.shift(-window_smooth) - series) / series
        Xdays_vol = Xdays_returns.rolling(window=window_smooth).std()
